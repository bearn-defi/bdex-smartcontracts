...
library UniswapV2OracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = IBdexPair(pair).price0CumulativeLast();
        price1Cumulative = IBdexPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IBdexPair(pair).getReserves();
        if (blockTimestampLast != blockTimestamp) {
            // subtraction overflow is desired
            uint32 timeElapsed = blockTimestamp - blockTimestampLast;
            (uint32 _tokenWeight0, uint32 _tokenWeight1) = IBdexPair(pair).getTokenWeights();
