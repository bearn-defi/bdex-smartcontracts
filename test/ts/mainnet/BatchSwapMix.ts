...
describe("BatchSwapMix", function () {
	...
	const prepareToken = async () => {
		...
		//deploy v2 factory
		const formula = await new BdexFormulaFactory(deployer).deploy();
		factory = await new BdexFactoryFactory(deployer).deploy(deployer.address, formula.address);
		controller = await new StakePoolControllerFactory(deployer).deploy();
		await controller.initialize(factory.address);
		router = await new BdexRouterFactory(deployer).deploy(factory.address, controller.address, wethAddress);
