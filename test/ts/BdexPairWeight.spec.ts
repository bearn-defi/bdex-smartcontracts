...
import {BdexFormula, BdexErc20, BdexFactory, BdexPair} from "../../typechain";
const overrides = {};

describe("BdexPairWeight", () => {
  let signers: SignerWithAddress[];

  let wallet: SignerWithAddress;
  let other: SignerWithAddress;

  let factory: BdexFactory;
  let token0: BdexErc20;
  let tokenA: BdexErc20;
  let formula: BdexFormula;
  let tokenWeight0: number;
  let token1: BdexErc20;
  let tokenB: BdexErc20;
  ...
  swapTestCases.forEach((swapTestCase, i) => {
    it(`getInputPrice:token0:${i}`, async () => {
      ...
      await expect(pair.swap(0, expectedOutputAmount.add(2), wallet.address, "0x", overrides)).to.be.revertedWith("BLP: K");
    ...
    it(`getInputPrice:token1:${i}`, async () => {
      ...
      await expect(pair.swap(amountOut.add(5), 0, wallet.address, "0x", overrides)).to.be.revertedWith("BLP: K");
  ...
  optimisticTestCases.forEach((optimisticTestCase, i) => {
    it(`optimistic:token0:${i}`, async () => {
      ...
      await expect(pair.swap(outputAmount.add(1), 0, wallet.address, "0x", overrides)).to.be.revertedWith("BLP: K");
    ...
    it(`optimistic:token1:${i}`, async () => {
      ...
      await expect(pair.swap(0, outputAmount.add(1), wallet.address, "0x", overrides)).to.be.revertedWith("BLP: K");
