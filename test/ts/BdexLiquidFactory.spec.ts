...
import {Erc20, TestErc20, TestErc20Factory, BdexErc20, BdexPairFactory} from "../../typechain";

let TEST_ADDRESSES: [string, string];

describe('BdexFactory', () => {
	...
	beforeEach(async () => {
		...
		uniswapPairBytecode = new BdexPairFactory(wallet).bytecode;
	...
	async function createPair(tokens: [string, string], tokenWeightA: number,
		...
		await expect(factory.createPair(...tokens)).to.be.reverted // BLP: PAIR_EXISTS
		await expect(factory.createPair(...tokens.slice().reverse())).to.be.reverted // BLP: PAIR_EXISTS
		...
		const pair = BdexPairFactory.connect(create2Address, wallet)
	...
	it('createPair', async () => {
		...
		await expect(factory.createPair(...TEST_ADDRESSES, 50, 30)).to.be.revertedWith('BLP: PAIR_EXISTS')
	...
	it('createInvalidPair', async () => {
		...
		], 50, 30)).to.be.revertedWith('BLP: ZERO_ADDRESS')
		await expect(createPair(TEST_ADDRESSES, 0, 30)).to.be.revertedWith('BLP: INVALID_TOKEN_WEIGHT')
		await expect(createPair(TEST_ADDRESSES, 100, 30)).to.be.revertedWith('BLP: INVALID_TOKEN_WEIGHT')
		await expect(createPair(TEST_ADDRESSES, 99, 30)).to.be.revertedWith('BLP: INVALID_TOKEN_WEIGHT')
		await expect(createPair(TEST_ADDRESSES, 51, 30)).to.be.revertedWith('BLP: INVALID_TOKEN_WEIGHT')
		await expect(createPair(TEST_ADDRESSES, 40, 0)).to.be.revertedWith('BLP: INVALID_SWAP_FEE')
		await expect(createPair(TEST_ADDRESSES, 40, 2000)).to.be.revertedWith('BLP: INVALID_SWAP_FEE')
		await expect(createPair(TEST_ADDRESSES, 40, 24)).to.be.revertedWith('BLP: INVALID_SWAP_FEE')
		await expect(createPair(TEST_ADDRESSES, 40, 0)).to.be.revertedWith('BLP: INVALID_SWAP_FEE')
	})
	...
	it('setFeeTo', async () => {
		await expect(factory.connect(other).setFeeTo(other.address)).to.be.revertedWith('BLP: FORBIDDEN')
		await factory.setFeeTo(wallet.address)
		expect(await factory.feeTo()).to.eq(wallet.address)
	})
	it('setProtocolFee', async () => {
		await expect(factory.connect(other).setProtocolFee(1)).to.be.revertedWith('BLP: FORBIDDEN')
		await expect(factory.setProtocolFee(1999)).to.be.revertedWith('BLP: Invalid Protocol fee')
		await expect(factory.setProtocolFee(100001)).to.be.revertedWith('BLP: Invalid Protocol fee')
	...
	it('setFeeToSetter', async () => {
		await expect(factory.connect(other).setFeeToSetter(other.address)).to.be.revertedWith('BLP: FORBIDDEN')
		await factory.setFeeToSetter(other.address)
		expect(await factory.feeToSetter()).to.eq(other.address)
		await expect(factory.setFeeToSetter(wallet.address)).to.be.revertedWith('BLP: FORBIDDEN')
	})
})
